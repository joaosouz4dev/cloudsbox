            ...
            <?php foreach ($results as $r) {
                ...
                echo '<td>';
                    if($this->permission->checkPermission($this->session->userdata('permissao'),'vArquivo')){
                        echo '<a class="btn btn-inverse " style="margin-right: 1%" target="_blank" href="'.$r->url.'" class="btn " title="Imprimir"><i class="icon-print"></i></a>'; 
                    }
                    if($this->permission->checkPermission($this->session->userdata('permissao'),'vArquivo')){
                        echo '<a href="'.base_url().'index.php/arquivos/download/'.$r->idDocumentos.'" class="btn " style="margin-right: 1%" title="Download"><i class="icon-download-alt"></i></a>'; 
                    }
                    if($this->permission->checkPermission($this->session->userdata('permissao'),'eArquivo')){ 
                        echo  '<a href="'.base_url().'index.php/arquivos/editar/'.$r->idDocumentos.'" class="btn btn-info " style="margin-right: 1%" title="Editar"><i class="icon-pencil icon-white"></i></a>';
                    }
                    if($this->permission->checkPermission($this->session->userdata('permissao'),'dArquivo')){
                         echo '<a href="#modal-excluir" style="margin-right: 1%" role="button" data-toggle="modal" arquivo="'.$r->idDocumentos.'" class="btn btn-danger " title="Excluir Arquivo"><i class="icon-remove icon-white"></i></a>';
                    }
                echo  '</td>';
                echo '</tr>';
            }?>
