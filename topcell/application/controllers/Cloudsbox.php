...
class Cloudsbox extends CI_Controller {


    /**
     * author: Jvsouza 
     * email: souza.joao1997@gmail.com
     * 
     */
    
    public function __construct() {
        parent::__construct();
        $this->load->model('cloudsbox_model','',TRUE);
        
    }

    public function index() {
        if( (!session_id()) || (!$this->session->userdata('logado'))){
            redirect('cloudsbox/login');
        }

        $this->data['ordens'] = $this->cloudsbox_model->getOsAbertas();
        $this->data['produtos'] = $this->cloudsbox_model->getProdutosMinimo();
        $this->data['os'] = $this->cloudsbox_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->cloudsbox_model->getEstatisticasFinanceiro();
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'cloudsbox/painel';
    ...
    public function minhaConta() {
        ...
        $this->data['usuario'] = $this->cloudsbox_model->getById($this->session->userdata('id'));
        $this->data['view'] = 'cloudsbox/minhaConta';
        $this->load->view('tema/topo',  $this->data);
    ...
    public function alterarSenha() {
        ...
        $result = $this->cloudsbox_model->alterarSenha($senha,$oldSenha,$this->session->userdata('id'));
        if($result){
            $this->session->set_flashdata('success','Senha Alterada com sucesso!');
            redirect(base_url() . 'index.php/cloudsbox/minhaConta');
        }
        else{
    ...
    public function pesquisar() {
        ...
        $data['results'] = $this->cloudsbox_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'cloudsbox/pesquisa';
        $this->load->view('tema/topo',  $this->data);
    ...
    public function login(){
        ...
        $this->load->view('cloudsbox/login');
    ...
    public function verificarLogin(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->model('Cloudsbox_model');
            $user = $this->Cloudsbox_model->check_credentials($email);
    ...
    public function emitente(){   
        ...
        $data['dados'] = $this->cloudsbox_model->getEmitente();
        $data['view'] = 'cloudsbox/emitente';
    ...
    public function cadastrarEmitente() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(base_url().'index.php/cloudsbox/emitente');
            ...
            $retorno = $this->cloudsbox_model->addEmitente($nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email, $logo);
            if($retorno){
    ...
    public function editarEmitente() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->cloudsbox_model->editEmitente($id, $nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email);
            if($retorno){
    ...
    public function editarLogo(){
        ...
        $retorno = $this->cloudsbox_model->editLogo($id, $logo);
        if($retorno){
